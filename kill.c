...
/*
 * Find the process using more than x% of memory and kill it.
 */
void kill_biggest_process(const poll_loop_args_t* args, int sig, long long MemTotalKiB)
{
    ...
    while (1) {
        ...
        if (cur.VmRSSkiB <= (double)MemTotalKiB * args->max_memory_per_process_percent / 100){
            // skip any process using less than 60% of the total memory
            debug("   \n");
            continue;   
        }
    ...
    for (int i = 0; i < 60; ++i) {
        if (sig != 0 || enable_debug) {
            warn("%d sending %s to process %d uid %d \"%s\": badness %d, VmRSS %lld MiB\n",
                i, sig_name, victim.pid, victim.uid, victim.name, victim.badness, victim.VmRSSkiB / 1024);
        }
        res = kill_wait(args, victim.pid, sig);
        if (res == 0) {
            break;
        }
    }
